import SigGolfCandidate.Hypertree.CachedPrepare
import SigGolfCandidate.Hypertree.ReusePrepare
namespace SigGolfCandidate.Hypertree.ConstantPrepare
...
def Code (image : Image) (p : Word) : Prop :=
  ...
  instructionAt image (p + 20) = some (.base (.ADD .x10 .x10 .x13)) ∧
...
theorem block (image : Image) (p : Word) (code : Code image p)
    ...
    OrdinarySteps image s 13 (state s) := by
  obtain ⟨c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12⟩ := code
  ...
  let s6 := execInstrBr s5 (.ADD .x10 .x10 .x13)
  ...
  change OrdinarySteps image s 13 s13
  apply OrdinarySteps.step s s1 _ (.base (.LD .x11 .x28 216)) 12
  ...
  apply OrdinarySteps.step s1 s2 _ (.base (.SD .x28 .x11 (-1048))) 11
  ...
  apply OrdinarySteps.step s2 s3 _ (.base (.LD .x11 .x28 224)) 10
  ...
  apply OrdinarySteps.step s3 s4 _ (.base (.SD .x28 .x11 (-1040))) 9
  ...
  apply OrdinarySteps.step s4 s5 _ (.base (.LD .x10 .x28 (-1080))) 8
  ...
  apply OrdinarySteps.step s5 s6 _ (.base (.ADD .x10 .x10 .x13)) 7
  · have hp : s5.pc = p + 20 := by simp [s1, s2, s3, s4, s5, execInstrBr, pc, BitVec.add_assoc]
    simpa only [fetch_at, hp] using c5
  · rfl
  apply OrdinarySteps.step s6 s7 _ (.base (.SD .x28 .x10 (-1080))) 6
  · have hp : s6.pc = p + 24 := by simp [s1, s2, s3, s4, s5, s6, execInstrBr, pc, BitVec.add_assoc]
    simpa only [fetch_at, hp] using c6
  · have hb : s6.getReg .x28 = 0x80438 := by
      simpa [s1, s2, s3, s4, s5, s6, execInstrBr, signExtend12, MachineState.getReg_setReg_eq, MachineState.getReg_setReg_ne] using base
    simp [s7, ordinaryStep, memoryArgumentsValid, hb, signExtend12, accessValid, rangeValid, MEMORY_BYTES]
  apply OrdinarySteps.step s7 s8 _ (.base (.ADDI .x28 .x28 (-1056))) 5
  · have hp : s7.pc = p + 28 := by simp [s1, s2, s3, s4, s5, s6, s7, execInstrBr, pc, BitVec.add_assoc]
    simpa only [fetch_at, hp] using c7
  · rfl
  apply OrdinarySteps.step s8 s9 _ (.base (.ADDI .x10 .x28 (-24))) 4
  ...
  · rfl
  apply OrdinarySteps.step s10 s11 _ (.base (.ADDI .x12 .x28 744)) 2
  ...
  · rfl
  apply OrdinarySteps.step s11 s12 _ (.base (.ADDI .x5 .x0 1)) 1
  ...
  · rfl
  apply OrdinarySteps.step s12 s13 _ (.base (.JAL .x0 48)) 0
...
theorem mem (s : MachineState) (base : s.getReg .x28 = 0x80438) (constant : s.getReg .x13 = 4294967296) (a : Word) :
    (state s).getMem a =
...
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.block' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms block
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.mem' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms mem
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.regs' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms regs
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.pc' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms pc
...
theorem header_memory_equiv (s : MachineState)
    (base : s.getReg .x28 = 0x80438) (constant : s.getReg .x13 = 4294967296) (ready : CachedPrepare.Ready s) (a : Word) :
    (state s).getMem a =
      (KeygenChainHeader.state (FusedPrepare.inputState s)).getMem a := by
  obtain ⟨h0,h1,h2,h3⟩ := ready
  rw [mem s base constant a, KeygenChainHeader.mem]
  simp only [input_mem]
...
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.header_memory_equiv' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
...
theorem frame (s : MachineState) (base : s.getReg .x28 = 0x80438) (constant : s.getReg .x13 = 4294967296)
    (a : Word) (h0 : a ≠ 0x80000) (h1 : a ≠ 0x80020) (h2 : a ≠ 0x80028) :
    (state s).getMem a = s.getMem a := by
  rw [mem s base constant a]
  simp only [h0,h1,h2,if_false]

/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.stack' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms stack
/-- info: 'SigGolfCandidate.Hypertree.ConstantPrepare.frame' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms frame
end SigGolfCandidate.Hypertree.ConstantPrepare
