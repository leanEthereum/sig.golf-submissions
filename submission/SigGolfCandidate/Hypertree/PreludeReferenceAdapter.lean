import SigGolfCandidate.Hypertree.PreludeBudgetExperiment
import SigGolfCandidate.Hypertree.SecurityBudget

namespace SigGolfCandidate.Hypertree.PreludeReferenceAdapter
open SigGolf OracleComp OracleSpec SecurityDerivation SecurityGameHop SecurityBytecode PreludeSecurity
...
theorem interaction (adversary : Adversary submission.sizes) (secretKey : SecretKey) (pk : PublicKey)
    ...
    project initial <$> resolve secretKey (SecurityBudget.counted (SecurityExperiment.interact adversary pk rounds state transcript)) =
      interactWith baseReferenceInterface adversary secretKey pk rounds state {transcript with hashCalls := initial} := by
...
theorem secretKeyed_program (adversary : Adversary submission.sizes) (rounds : Nat) (secretKey : SecretKey) :
    project 0 <$> resolve secretKey (SecurityBudget.counted (SecurityExperiment.program KeygenFunctional.zeroCache adversary rounds)) =
      secretKeyedWith baseReferenceInterface adversary rounds secretKey := by
  unfold SecurityExperiment.program
  rw [project_bind, resolve_counted_split, SecurityIdealKeygen.simulate_keygen]
  simp only [secretKeyedWith, baseReferenceInterface, referenceKeygen, OracleComp.liftComp_bind,
...
/-- info: 'SigGolfCandidate.Hypertree.PreludeReferenceAdapter.reference_experiment' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms reference_experiment
end SigGolfCandidate.Hypertree.PreludeReferenceAdapter
