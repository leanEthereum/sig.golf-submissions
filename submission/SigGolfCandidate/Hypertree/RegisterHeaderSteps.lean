import SigGolfCandidate.Hypertree.RegisterHeaderInitialData
import SigGolfCandidate.Hypertree.CounterCheck
namespace SigGolfCandidate.Hypertree.RegisterHeaderSteps
...
def InitialCode (image : Image) (p : Word) : Prop :=
  RegisterHeader.InitialCode image p ∧ RegisterHeaderCore.Code image (p+236)
def RecurrentCode (image : Image) (p : Word) : Prop :=
  RegisterHeader.PrepareCode image p ∧ RegisterHeaderCore.Code image (p+104)
...
theorem initial (image : Image) (hash : Hash)
    ...
    (prepareCode : RegisterHeader.InitialCode image 0x1500) (coreCode : RegisterHeaderCore.Code image 0x15ec)
  ...
  obtain ⟨final,run,finalPC,finalData,finalHeaderReady,finalReady,finalBase,finalConstant,finalArgs,finalCounter,finalLimit,ra,sp,frame⟩ :=
    RegisterHeaderInitialData.compute image hash 0x1500 prepareCode coreCode (CheckReuse.shortCheck s)
  ...
  refine ⟨final,(CheckReuse.block image 0x14f4 checkCode s pc base).trace.trans run,finalPC,
    finalData,finalHeaderReady,finalReady,finalBase,finalConstant,finalArgs,finalCounter,finalLimit.trans (PersistentLimit.initial_installs s),ra.trans (CheckReuse.short_stack s).1,
    sp.trans (CheckReuse.short_stack s).2,?_⟩
  intro a outside
  rw [frame a outside,CheckReuse.short_mem]

/-- info: 'SigGolfCandidate.Hypertree.RegisterHeaderSteps.initial' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms initial
theorem recurrent (image : Image) (hash : Hash)
    (checkCode : CounterCheck.Code image 0x1580)
    (prepareCode : RegisterHeader.PrepareCode image 0x1584) (coreCode : RegisterHeaderCore.Code image 0x15ec)
    (s : MachineState) (level tree step : Nat) (side : Bool) (chain : Reference.Chain) (value : Reference.Digest)
    (pc : s.pc = 0x1580) (base : s.getReg .x28 = 0x80438)
    (constant : s.getReg .x13 = 4294967296) (ready : CachedPrepare.Ready s) (headerReady : RegisterHeader.Ready s)
    ...
    ∃ final, Trace hash image s 9 16 1 1 final ∧ final.pc = 0x1580 ∧
      Buffered final level tree side chain (step+1) (Reference.chainHash hash level tree side chain step value) ∧
      RegisterHeader.Ready final ∧ CachedPrepare.Ready final ∧ final.getReg .x28 = 0x80438 ∧ final.getReg .x13 = 4294967296 ∧
      (final.getReg .x11 = 48 ∧ final.getReg .x12 = 0x80020 ∧ final.getReg .x5 = 1) ∧
      final.getReg .x6 = final.getMem 0x80438 ∧
  ...
  obtain ⟨final,run,finalPC,finalData,finalHeaderReady,finalReady,finalBase,finalConstant,finalArgs,finalCounter,finalLimit,ra,sp,frame⟩ :=
    RegisterHeaderData.recurrent image hash 0x1584 prepareCode coreCode (CounterCheck.shortCheck s)
      level tree step side chain value checkedPC (by rw [CounterCheck.short_base]; exact base)
      ((check_constant s).trans constant) (CounterCheck.ready s ready) (RegisterHeader.check_ready s headerReady)
  ...
  refine ⟨final,(CounterCheck.block image 0x1580 checkCode s pc).trace.trans run,finalPC,
    finalData,finalHeaderReady,finalReady,finalBase,finalConstant,finalArgs,finalCounter,finalLimit.trans ((PersistentLimit.check_preserves s .x7).trans limit),ra.trans (CounterCheck.short_stack s).1,
    sp.trans (CounterCheck.short_stack s).2,?_⟩
  intro a outside
  rw [frame a outside,CounterCheck.short_mem]

/-- info: 'SigGolfCandidate.Hypertree.RegisterHeaderSteps.recurrent' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs (whitespace := lax) in
#print axioms recurrent
end SigGolfCandidate.Hypertree.RegisterHeaderSteps
