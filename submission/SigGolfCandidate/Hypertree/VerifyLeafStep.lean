...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (9*(7-(Reference.digit message chain).val)+registerHeaderOverhead (7-(Reference.digit message chain).val)+44)
      (16*(7-(Reference.digit message chain).val)+registerHeaderOverhead (7-(Reference.digit message chain).val)+44)
