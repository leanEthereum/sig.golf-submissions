...
namespace SigGolfCandidate.Hypertree.InplaceCore
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 Keygen Signing
set_option maxRecDepth 8192

def Code (image : Image) (p : Word) : Prop :=
  instructionAt image p = some (.base .ECALL) ∧ InplaceFinish.Code image (p+4)
...
theorem compute (image : Image) (hash : Hash) (p : Word) (code : Code image p)
    ...
    ∃ final, Trace hash image s 6 13 1 1 final ∧ final.pc = p-108 ∧
  ...
  · intro i
    rw [InplaceFinish.mem _ hashBase, if_neg (by fin_cases i <;> decide)]
  ...
  · intro a outside notStep
    rw [InplaceFinish.mem _ hashBase,if_neg notStep]
    exact InplaceHash.frame s _ destination a outside

/-- info: 'SigGolfCandidate.Hypertree.InplaceCore.compute' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms compute
end SigGolfCandidate.Hypertree.InplaceCore
