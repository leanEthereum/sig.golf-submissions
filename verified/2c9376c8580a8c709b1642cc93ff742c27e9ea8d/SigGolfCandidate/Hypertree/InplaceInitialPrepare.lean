...
namespace SigGolfCandidate.Hypertree.InplaceInitialPrepare
...
def Code (image : Image) (p : Word) : Prop :=
  ...
  instructionAt image (p + 108) = some (.base (.ADDI .x12 .x28 8)) ∧
...
theorem block (image : Image) (p : Word) (code : Code image p)
  ...
  let s28 := execInstrBr s27 (.ADDI .x12 .x28 8)
  ...
  · rfl
  apply OrdinarySteps.step s27 s28 _ (.base (.ADDI .x12 .x28 8)) 4
...
def beforeArgs (s : MachineState) : MachineState :=
  let s := execInstrBr s (.LD .x11 .x28 (216))
  let s := execInstrBr s (.SD .x28 .x11 (-1048))
  let s := execInstrBr s (.LD .x11 .x28 (224))
  let s := execInstrBr s (.SD .x28 .x11 (-1040))
...
theorem before_base (s : MachineState) (base : s.getReg .x28 = 0x80438) :
    (beforeArgs s).getReg .x28 = 0x80018 := by
  simp [beforeArgs, execInstrBr, signExtend12, base,
    MachineState.getReg_setReg_eq, MachineState.getReg_setReg_ne]

theorem tail_equiv (s : MachineState) (base : s.getReg .x28 = 0x80018) :
    newTail s = (oldTail s).setReg .x12 0x80020 := by
  cases s with
  | mk regs mem code pc committed publicValues privateInput inputBufBase =>
    ...
    simp [newTail, oldTail, execInstrBr, MachineState.getReg, MachineState.setReg,
      MachineState.setPC, signExtend12, signExtend21, base, BitVec.add_assoc]
    funext r; cases r <;> rfl

theorem state_equiv (s : MachineState) (base : s.getReg .x28 = 0x80438) :
    state s = ((KeygenChainHeader.state (FusedPrepare.inputState s)).setReg .x13 4294967296).setReg .x12 0x80020 := by
  calc
    state s = (ConstantInitialPrepare.state s).setReg .x12 0x80020 :=
      tail_equiv (beforeArgs s) (before_base s base)
    _ = _ := by rw [ConstantInitialPrepare.state_equiv s base]

/-- info: 'SigGolfCandidate.Hypertree.InplaceInitialPrepare.block' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms block
/-- info: 'SigGolfCandidate.Hypertree.InplaceInitialPrepare.tail_equiv' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms tail_equiv
/-- info: 'SigGolfCandidate.Hypertree.InplaceInitialPrepare.state_equiv' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms state_equiv
end SigGolfCandidate.Hypertree.InplaceInitialPrepare
