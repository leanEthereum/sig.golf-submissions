import SigGolfCandidate.Hypertree.PreludeLoopBlocks
...
theorem sign_layer_prepare (hash : Hash) (s : MachineState) (secretKey : SecretKey) (level index : Nat)
    ...
    ∃ ready, Trace hash signPrelude s (if level = 0 then 34 else 489) (if level = 0 then 34 else 489) 0 0 ready ∧
  ...
  obtain ⟨ready,dispatch,readyPC,readyRA,readySP,digits,frame⟩ :=
    dispatch_to_tree signPrelude 0x1294 sign_dispatch_code sign_encode_code (by decide) shifted current shiftedPC shiftedSP currentLo currentHi
