...
theorem leaf_step (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final, Trace hash verify s (10*(7-(Reference.digit message chain).val)+stepBaseOverhead (7-(Reference.digit message chain).val)+44)
      (17*(7-(Reference.digit message chain).val)+stepBaseOverhead (7-(Reference.digit message chain).val)+44)
