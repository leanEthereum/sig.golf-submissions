import SigGolfCandidate.Hypertree.PreludeLayer
import SigGolfCandidate.Hypertree.SignLayers

namespace SigGolfCandidate.Hypertree.Signing.Prelude
open SigGolf SigGolf.Riscv RiscvZkvm.Rv64 OracleComp Keygen Verifying
...
theorem sign_layers (hash : Hash) (secretKey : SecretKey) (count : Nat) :
    ...
    ∃ final instructions cycles, Trace hash signPrelude s instructions cycles (loopCalls count level) (loopBlocks count level) final ∧
  ...
  | zero =>
    ...
    · simpa [loopCalls,loopBlocks] using Trace.refl (hash := hash) (image := signPrelude) s
...
/-- info: 'SigGolfCandidate.Hypertree.Signing.Prelude.sign_layers' depends on axioms: [propext, Classical.choice, Quot.sound] -/
