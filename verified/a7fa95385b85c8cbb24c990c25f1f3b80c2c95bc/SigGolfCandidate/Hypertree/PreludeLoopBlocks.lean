...
import SigGolfCandidate.Hypertree.PreludeTree
...
theorem shiftIndexState_block (s : MachineState) (pc : s.pc = 0x1220) :
    OrdinarySteps signPrelude s 29 (shiftIndexState s) := by
...
theorem sign_dispatch_code : DispatchCode signPrelude 0x1294 := by decide


theorem sign_encode_code : EncodeCode signPrelude 0x1340 := prelude_encode_code

theorem sign_advance_code : AdvanceCode signPrelude 0x12ac := by
  intro s i pc
  simp only [fetch, pc]
  fin_cases i <;> decide
