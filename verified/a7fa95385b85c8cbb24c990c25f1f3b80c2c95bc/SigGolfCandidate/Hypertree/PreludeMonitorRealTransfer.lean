...
import SigGolfCandidate.Hypertree.PreludeReferenceCutoff
...
import SigGolfCandidate.Hypertree.PreludeMonitorWinBound

namespace SigGolfCandidate.Hypertree.PreludeMonitorRealTransfer
...
theorem real_le_factor_union (publicCache : SigGolf.Cache) (adversary : Adversary submission.sizes)
    (rounds budget : Nat) :
    Pr[PreludeReferenceCutoff.Won | PreludeReferenceCutoff.experiment publicCache adversary rounds budget] ≤
      Pr[fun result => PreludeReferenceCutoff.Won result.2.1 ∨ SecretKeyHitTrace result.2.2 result.1 |
        (do let secretKey ← sampleSecretKey
            let trace ← factorTrace publicCache adversary rounds budget
            pure (secretKey, trace))] := by
  simp only [PreludeReferenceCutoff.experiment, bind_pure_comp, probEvent_bind_eq_tsum,
    probEvent_map, Function.comp_def]
  apply ENNReal.tsum_le_tsum
  intro secretKey
  apply mul_le_mul' le_rfl
  have equality := probEvent_congr'
    (p := fun result : TraceResult => PreludeReferenceCutoff.Won result.1 ∨ SecretKeyHitTrace result.2 secretKey)
    (q := fun result : TraceResult => PreludeReferenceCutoff.Won result.1 ∨ SecretKeyHitTrace result.2 secretKey)
  ...
  exact SecuritySecretKeyStoppedTrace.real_cutoff_le_ideal_union secretKey
    (SecurityExperiment.program publicCache adversary rounds) budget PreludeReferenceCutoff.Won
...
theorem real_le_true_union (publicCache : SigGolf.Cache) (adversary : Adversary submission.sizes)
    (rounds budget : Nat) :
    Pr[PreludeReferenceCutoff.Won | PreludeReferenceCutoff.experiment publicCache adversary rounds budget] ≤
      Pr[fun result => PreludeReferenceCutoff.Won result.value.2.value ∨
  ...
  have eventEq := probEvent_congr'
    (p := fun result => PreludeReferenceCutoff.Won result.value.2.value ∨
      SecuritySecretKeyMonitor.Hit result.value.2.history.secretKeyInputs result.secretKey)
    (q := fun result => PreludeReferenceCutoff.Won result.value.2.value ∨
      SecuritySecretKeyMonitor.Hit result.value.2.history.secretKeyInputs result.secretKey)
...
theorem real_le_common_union (publicCache : SigGolf.Cache) (adversary : Adversary submission.sizes)
    ...
    Pr[PreludeReferenceCutoff.Won | PreludeReferenceCutoff.experiment publicCache adversary rounds budget] ≤
      Pr[fun result => SecurityMonitorCombined.GraphBad result ∨ SecurityMonitorCombined.SecretKeyBad result ∨
        PreludeReferenceCutoff.Won result.value.2.value.value |
  ...
  have bound := SecurityMonitorGraphMixture.experiment_le_union publicCache adversary rounds budget
    (fun secretKey result => PreludeReferenceCutoff.Won result.2.value ∨
...
theorem real_le_bad (publicCache : SigGolf.Cache) (adversary : Adversary submission.sizes)
  ...
  · exact PreludeMonitorWin.supported_won_bad publicCache adversary rounds budget result member won
...
/-- info: 'SigGolfCandidate.Hypertree.PreludeMonitorRealTransfer.real_bound' depends on axioms: [propext,
 Classical.choice,
 Quot.sound] -/
#guard_msgs in
#print axioms real_bound
end SigGolfCandidate.Hypertree.PreludeMonitorRealTransfer
