...
namespace SigGolfCandidate.Hypertree.PreludeMonitorWin
open SecurityMonitorWin
...
set_option linter.constructorNameAsVariable false
 ...
 theorem joint_won_bad (publicCache : Cache) (adversary : Adversary submission.sizes)
    ...
    (won : PreludeReferenceCutoff.Won result.2.value.value) :
...
theorem supported_won_bad (publicCache : Cache) (adversary : Adversary submission.sizes)
    ...
    (won : PreludeReferenceCutoff.Won result.value.2.value.value) : SecurityMonitorCombined.Bad result := by
...
theorem won_le (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
    Pr[fun result => PreludeReferenceCutoff.Won result.value.2.value.value |
...
/-- info: 'SigGolfCandidate.Hypertree.PreludeMonitorWin.won_le' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms won_le
end SigGolfCandidate.Hypertree.PreludeMonitorWin
