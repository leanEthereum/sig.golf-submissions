...
namespace SigGolfCandidate.Hypertree.RegisterHeaderCore
...
theorem compute (image : Image) (hash : Hash) (p : Word) (code : Code image p)
    ...
    (counter : s.getReg .x6 = s.getMem 0x80438) (headerReady : RegisterHeader.Ready s)
    ...
    ∃ final, Trace hash image s 4 11 1 1 final ∧ final.pc = p-108 ∧
      ...
      RegisterHeader.Ready final ∧ CachedPrepare.Ready final ∧ final.getReg .x28 = 0x80438 ∧
  ...
  refine ⟨StepBaseFinish.state hashed, hashTrace.trans tail.trace, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
  ...
  · exact RegisterHeader.finish_ready hashed hashBase (RegisterHeader.hash_ready s _ destination headerReady)
...
/-- info: 'SigGolfCandidate.Hypertree.RegisterHeaderCore.compute' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms compute
end SigGolfCandidate.Hypertree.RegisterHeaderCore
