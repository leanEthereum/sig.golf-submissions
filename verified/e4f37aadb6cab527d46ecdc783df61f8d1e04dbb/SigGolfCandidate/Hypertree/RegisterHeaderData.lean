...
import SigGolfCandidate.Hypertree.RegisterHeaderCore
import SigGolfCandidate.Hypertree.RegisterHeaderBlocks
import SigGolfCandidate.Hypertree.CounterArgs
namespace SigGolfCandidate.Hypertree.RegisterHeaderData
...
theorem recurrent (image : Image) (hash : Hash) (p : Word)
    (prepareCode : RegisterHeader.PrepareCode image p) (coreCode : RegisterHeaderCore.Code image (p+104))
    ...
    (ready : CachedPrepare.Ready s) (headerReady : RegisterHeader.Ready s)
    ...
    ∃ final, Trace hash image s 8 15 1 1 final ∧ final.pc = p-4 ∧
      Buffered final level tree side chain (step+1) (Reference.chainHash hash level tree side chain step value) ∧
      RegisterHeader.Ready final ∧ CachedPrepare.Ready final ∧ final.getReg .x28 = 0x80438 ∧ final.getReg .x13 = 4294967296 ∧
  ...
  let prepared := (((InplacePrepare.state s).setReg .x28 0x80438).setPC (s.pc+104)).setReg .x14 (s.getReg .x14+s.getReg .x13)
  have prepPC : prepared.pc = p+104 := by simp only [prepared,MachineState.setPC,MachineState.setReg,pc]
  ...
  have prepHeaderReady : RegisterHeader.Ready prepared := by
    have h := RegisterHeader.prepare_ready s base
    rw [RegisterHeader.prepare_equiv s base headerReady, PersistentStepBase.prepare_equiv s base,CounterArgs.equiv s base args.1 args.2.1 args.2.2] at h
    simpa only [resetPC] using h
  obtain ⟨final,core,finalPC,valueEq,finalHeaderReady,finalReady,finalBase,finalConstant,finalArgs,stepEq,finalCounter,finalLimit,ra,sp,frame⟩ :=
    RegisterHeaderCore.compute image hash (p+104) coreCode prepared prepPC
      (by simp [prepared,MachineState.getReg_setPC,MachineState.getReg_setReg_eq,MachineState.getReg_setReg_ne]) prepConstant current
      ...
      prepCounter prepHeaderReady level tree step side chain value words
  ...
  have prepTrace := RegisterHeader.prepare_block image p prepareCode s pc base
  rw [RegisterHeader.prepare_equiv s base headerReady,PersistentStepBase.prepare_equiv s base,CounterArgs.equiv s base args.1 args.2.1 args.2.2] at prepTrace
  have prepTraceExact : OrdinarySteps image s 4 prepared := by
    simpa only [resetPC] using prepTrace
  refine ⟨final,prepTraceExact.trace.trans core,?_,?_,finalHeaderReady,finalReady,
...
/-- info: 'SigGolfCandidate.Hypertree.RegisterHeaderData.recurrent' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms recurrent
end SigGolfCandidate.Hypertree.RegisterHeaderData
