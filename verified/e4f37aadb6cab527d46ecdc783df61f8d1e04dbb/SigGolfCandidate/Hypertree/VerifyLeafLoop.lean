...
/-- All remaining verifier chains finish, with their exact recovered endpoints and uniform resource bounds. -/
theorem leaf_loop (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 721*remaining ∧ cycles + 16* ChainCost.chainPrefix message start ≤ 16* ChainCost.chainPrefix message 46 + 81*remaining ∧ calls ≤ 7*remaining ∧ final.pc = 0x1690 ∧
  ...
  | zero =>
    ...
    subst start
    refine ⟨s, 0, 0, 0, Trace.refl s, by omega, by omega, by omega, ?_, ?_, data, completed, rfl, rfl, ?_⟩
    · simpa using pc
    · exact counter
    · intro _ _; rfl
  | succ remaining ih =>
    ...
    have overheadBound := registerHeaderOverhead_le (7-(Reference.digit message chain).val)
    refine ⟨final, (9*(7-(Reference.digit message chain).val)+registerHeaderOverhead (7-(Reference.digit message chain).val)+44)+steps,
      (16*(7-(Reference.digit message chain).val)+registerHeaderOverhead (7-(Reference.digit message chain).val)+44)+cycles,
...
theorem recover_all_chains (hash : Hash) (s : MachineState) (level tree : Nat) (side : Bool) (base : Nat)
    ...
    ∃ final steps cycles calls, Trace hash verify s steps cycles calls calls final ∧
      steps ≤ 33166 ∧ cycles ≤ 8654 ∧ calls ≤ 322 ∧ final.pc = 0x1690 ∧
      final.getMem 0x80430 = 46 ∧ LeafData final level tree side base message values ∧
...
#guard_msgs in
#print axioms recover_all_chains
